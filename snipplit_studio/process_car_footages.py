...
print("Processing car footages from Downloads...")
processed_files = []

for file_path in source_dir.glob("*.mp4"):
    filename = file_path.name
    clean_name = filename if filename.startswith("car_") else f"car_{filename}"
    ...
    print(f"[+] Added car footage #{footage_id}: {clean_name} ({int(duration)}s, {width}x{height})")

print(f"SUCCESS: Imported {len(processed_files)} car footages into database and target folder!")
