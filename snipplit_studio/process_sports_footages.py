...
print("Processing sports footages from Downloads...")
processed_files = []

for file_path in source_dir.glob("*.mp4"):
    filename = file_path.name
    clean_name = filename if (filename.startswith("football_") or filename.startswith("sport_")) else f"football_{filename}"
    ...
    print(f"[+] Added sports footage #{footage_id}: {clean_name} ({int(duration)}s, {width}x{height})")

print(f"SUCCESS: Imported {len(processed_files)} sports footages into database and target folder!")
